...
// Pick exported hook if present: export function useI18n(){ return { t, lang } }
...
// Build portfolio timeline: evaluate value at each transaction date and at asOf
function computePortfolioSeries(rows: Tx[], scenario: Scenario, asOf: string, growthFactor: number) {
  ...
  // Unique sorted dates consisting of all tx dates and the asOf date
...
export default function ImportPage() {
  ...
  const [growthFactor, setGrowthFactor] = useState<number>(1); // 1.0x baseline ("demo" growth slider)
  ...
  // Per-row what-if at asOf (for the table)
  ...
  // Portfolio timelines (base vs adjusted), same y-axis domain
  ...
  const yMin = useMemo(() => Math.min(...(baseTimeline.values.length ? baseTimeline.values : [0]), ...(adjustedTimeline.values.length ? adjustedTimeline.values : [0])), [baseTimeline, adjustedTimeline]);
  const yMax = useMemo(() => Math.max(1, ...(baseTimeline.values.length ? baseTimeline.values : [1]), ...(adjustedTimeline.values.length ? adjustedTimeline.values : [1])), [baseTimeline, adjustedTimeline]);
    ...
    <main className="min-h-screen">
      <div className="mx-auto max-w-5xl px-4 py-10">
        ...
        <section className="mb-10 rounded-2xl border border-gray-200 p-6 shadow-sm">
          ...
          {/* Base chart (teal, portfolio timeline) */}
          <div className="mt-6">
        ...
        <section className="rounded-2xl border border-gray-200 p-6 shadow-sm">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="text-sm">
              ...
              <div><span className="text-gray-600">{t('adjusted_hypothetical')}</span> <strong>{formatEUR(adjustedTimeline.values.at(-1) ?? 0)}</strong></div>
            </div>
          </div>

          {/* Adjusted chart (teal, portfolio timeline), same y-axis as base */}
          <div className="mt-6">
            <div className="rounded-2xl border p-4">
